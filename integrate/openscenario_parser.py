...
# ⚠️ This script is supposed to replace the standard <openscenario_parser.py> script
# Resource: `scenario_runenr-0.9.15 > srunner > tools > openscenario_parser.py`
# Any changes that deviate from the original are inside "Custom Begin" / "Custom End" marked area
        ...
        if condition.find('ByEntityCondition') is not None:
            ...
            if value_condition.find('ParameterCondition') is not None:
                ...
                if condition_name.startswith('criteria_'):
                    ...
                    ## Custom Begin -----------------------------------------------------------------------------------
                    from srunner.scenariomanager.scenarioatomics.atomic_custom_criteria import CustomFinallyInTargetArea

                    if (condition_name == "criteria_CustomFinallyInTargetArea"):

                        if (arg_name != "min_x,max_x,min_y,max_y"):
                            raise AttributeError("CustomFinallyInTargetArea only supports parameterRef <min_x,max_x,min_y,max_y>")
                        
                        values = [ float(value) for value in value.reference_text.split(',') ] # min_x,max_x,min_y,max_y
                        if len(values) != 4:
                            raise AttributeError("criteria_CustomFinallyInTargetArea: values of parameterRef must be in format: min_x,max_x,min_y,max_y!")
                        
                        atomic = py_trees.composites.Parallel("Evaluation Criteria for multiple ego vehicles")
                        
                        for triggered_actor in actor_list:
                            atomic.add_child(CustomFinallyInTargetArea(triggered_actor, values[0], values[1], values[2], values[3]))
